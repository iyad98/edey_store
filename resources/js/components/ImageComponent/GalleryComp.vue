        ...
        <div class="modal-dialog modal-xl" role="document" style="max-width: 84%;">
            <div class="modal-content">
                        ...
                        <div class="col-sm-7 gallery_images vertical-line-left">
                            ...
                            <div class="row">
                                <div class="pointer col-sm-3 mt-3" v-for="image in gallery.images"
                                     :class="image.id == selected_image.id ? 'image-selected' : ''"
                                     @click="setSelectedImage(image)">
                                    <img v-show="image.mime_type.includes('image')"
                                          width="120"
                                         height="100" :src="image.mime_type.includes('image') ? image.src : ''"/>

                                    <video v-show="!image.mime_type.includes('image')"
                                           width="120" height="100"
                                           >
                                        <source :src="!image.mime_type.includes('image') ? image.src : '' " type="video/mp4">
                                        <source :src="!image.mime_type.includes('image') ? image.src : '' " type="video/ogg">
                                    </video>
                                </div>
                            </div>
                            <div class="row mt-4">
                        ...
                        <div class="col-sm-2">
                            ...
                            <div class="row mt-2">
                                <div class="col-sm-12 word-break" >
                                    <label><span>الاسم : </span> : {{selected_image.name}}</label>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="modal-footer">
                    ...
                    <!--{{__('admin.cancel')}}-->
                    <!--</button>-->
                    <button @click="sendFile" :disabled="!active_save" type="button" style="width: 120px;"
...
<script>
    export default {
        ...
        props: ['default_image', 'attr_name', 'selector_id_image' , 'obj'],
        data: function () {
            return {
                gallery: {
                    images: [],
        ...
        computed : {
            active_save : function () {
                return typeof this.selected_image.id == 'number' || this.selected_image.file != null;
            }  ,
        },
        methods: {
            getTypes: function () {
            ...
            sendFile: function () {
                this.$emit('get-advance-emit-file',this.obj , this.selected_image , this.attr_name);
              //  $('#'+this.selector_id_image).prop('src' ,this.selected_image.src );
                if(this.check_image(this.selected_image.src)) {
                    $('#'+this.selector_id_image).prop('src' , this.selected_image.src);
                    ...
                    vid.src = this.selected_image.src;
                    vid.load();
