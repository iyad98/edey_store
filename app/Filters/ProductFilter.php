...
class ProductFilter extends BaseFilter
{
    ...
    public function filter_by_name($value)
    {
        ...
        if ($value != -1) {
            $this->builder->where(function ($query) use ($value) {
                ...
                $query->where('products.name_en', 'LIKE', "%$value%")
                    ->orWhere('products.name_ar', 'LIKE', "%$value%")
                    ->orWhereIn('products.id' , function ($query2) use($value){
                        $query2->from('product_variations')->select('product_id')->where('sku' ,'LIKE', "%$value%");
                    });
            });
        }
    ...
    public function filter_by_brand($value)
    {
        if ($value != -1 && !empty($value) && !is_null($value)) {
            $this->builder->where('products.brand_id', '=', $value);
        }
    ...
    public function filter_by_is_variation($value)
    {
        if ($value != -1 && !empty($value) && !is_null($value)) {
            if ($value == 1) {
                $this->builder->where('products.is_variation', '=', 0);
            } else {
                $this->builder->where('products.is_variation', '=', 1);
            }
        }
...
//                $query->whereIn('product_categories.category_id', $value);
//            });
            $this->builder->whereIn('products.id' , function ($query) use($value){
                $query
    ...
    public function filter_by_stock_status($value)
    {
        if ($value != -1 && !empty($value) && !is_null($value)) {
            if ($value == 1) {
                $this->builder->whereHas('all_variations', function ($query) use ($value) {
                    $query->where('stock_status_id', '=', 1);
                });
            } else {
                $this->builder->whereDoesntHave('all_variations', function ($query) use ($value) {
                    $query->where('stock_status_id', '=', 1);
                });
            }
